...
@Component({
  selector: 'app-owners-reservations-page',
  templateUrl: './owners-reservations-page.component.html',
  styleUrls: ['./owners-reservations-page.component.css'],
})
export class OwnersReservationsPageComponent {
  res: ReservationDTO[];
  ...
  ngOnInit() {
    ...
    this.route.params.subscribe((params) => {
      ...
      this.service.getUserByEmail(userEmail).subscribe({
        next: (data: User) => {
          if (Role[Role[data.role]] == 'OWNER') {
            this.reservationService
              .getCurrentOwnersReservations(data.id)
              .subscribe((data) => {
                this.res = data;
              });
          } else {
            this.snackBar.open('Logged in user is not an owner', 'Close', {
