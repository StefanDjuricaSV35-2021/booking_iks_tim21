...
import { ActivationRequest } from 'src/app/profile/model/activationRequest.model';
...
export class ActivateAccountComponent implements OnInit {
  ...
  ngOnInit(){
    ...
    if(e == null){
      ...
      this.userService.getActivationRequest(e).subscribe({
        ...
        error: (error) => {
          console.error('Failed to load page duo to invalid or expired request:', error);
  ...
  activate(){
    this.userService.activateAccount(this.email).subscribe({
      next: (data: User) => {
        alert("You have successfully activated your account you will be redirected to login page.")
